...
def embed_text(text, model):
    # Tokenize and embed each word in the text
    embeddings = [model[word] for word in word_tokenize(text.lower()) if word in model]
    # Average the embeddings for the whole text
    if len(embeddings) > 0:
        return np.mean(embeddings, axis=0)
    else:
        return np.zeros(model.vector_size)

def process_data(label_num=2, max_features=None, seed=0):
    ...
    # Create paper features using GloVe embeddings
    ...
    paper_features = np.array([embed_text(text, glove_model) for text in papers_df['combined_text']])
